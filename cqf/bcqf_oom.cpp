...
result_query Bcqf_oom::query(string seq, int k){
    int s = (this->quotient_size + this->remainder_size) / 2;
    int z = k-s;
    int n = seq.length();
    ...
    for (auto i = s+z-1; i < n; i++){
        current_smer <<= 2;
        current_smer = (current_smer | nucl_encode(seq[i])) & mask_right(2*s);
        last_smers_abundances[(i-1)%(z+1)] = this->query(bfc_hash_64(flip(canonical(current_smer, 2*s), 2*s), mask_right(s*2)));
